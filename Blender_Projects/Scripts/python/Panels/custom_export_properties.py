...
class MESH_OT_add_subdiv_monkey(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.context.object["Object Tag"]       = str(bpy.context.scene.my_tool.tag_enum)
        bpy.context.object["Object Layer"]     = str(bpy.context.scene.my_tool.layer_enum)
        bpy.context.object["Object Type"]      = str(bpy.context.scene.my_tool.type_enum)
        bpy.context.object["Object Attribute"]        = str(bpy.context.scene.my_tool.attribute_enum)
        bpy.context.object["Object Component"]        = str(bpy.context.scene.my_tool.component_enum)
        bpy.context.object["Object Interaction"]         = str(bpy.context.scene.my_tool.interact_enum)
        
        bpy.context.object["Category"]         = str(bpy.context.scene.my_tool.category_enum)
        bpy.context.object["SubCategory"]      = str(bpy.context.scene.my_tool.subcategory_enum)
        bpy.context.object["Classification"]   = int(bpy.context.scene.my_tool.classification_enum)
        bpy.context.object["Standardization"]  = int(bpy.context.scene.my_tool.standardization_enum)
...
class VIEW3D_PT_my_custom_panel(bpy.types.Panel):  # class naming convention ‘CATEGORY_PT_name’
    ...
    def draw(self, context):
        ...
        col = split.column(align=True)
        col.label(text="")
        col.operator("object.empty_add", text="Place Locator", icon="EMPTY_DATA")

        row = layout.row()
        row.operator("mesh.primitive_ico_sphere_add", text="Set Component", icon="FILE_SCRIPT")
        
        row.operator("object.shade_smooth", text="Create Prefab", icon="OUTLINER")
        
        row = layout.row()
        row.operator("mesh.add_unity_attributes", text="Set Attributes", icon="RNA_ADD")
        
        layout.separator()
        
        row = layout.row(align=True)
        row.operator("object.send_to_unity", text="Send to Unity", icon="CON_TRACKTO")
        row.operator("object.return_to_unity", text="Return to Unity", icon="FILE_REFRESH")
